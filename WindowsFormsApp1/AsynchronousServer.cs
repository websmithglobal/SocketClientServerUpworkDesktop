...
namespace WindowsFormsApp1
{
    class AsynchronousServer
    {
        ...
        public static void Send(String data, int without)
        {
            ...
            if (without == -1)
            {
                Form1.SetControlPropertyThreadSafe(console, "Text", console.Text + "Server: " + data + "\n");
                data += "<EOF>\n";
            }
            ...
            if (runningServer)
            {
                try
                {
                    ...
                    if (runningServer)
                    {
                        server.Close();
                        runningServer = false;
                        Form1.SetControlPropertyThreadSafe(console, "Text", console.Text + "Server closed\n");
                    }      
                }
                catch (Exception e)
                {
                    Form1.SetControlPropertyThreadSafe(console, "Text", console.Text + "Error trying to close server: " + e.ToString() + "\n");
                }
            }
    ...
    class Client
    {
        ...
        public Client(Socket client, int count)
        {
            ...
            Form1.SetControlPropertyThreadSafe(AsynchronousServer.console, "Text", AsynchronousServer.console.Text + "Client connected: " + client.RemoteEndPoint.ToString() + "\n");
            ...
            chatThread = new Thread(() =>
            {
                ...
                while (connected)
                {
                    try
                    {
                        ...
                        if (receivedBytes == 0)
                        {
                            Form1.SetControlPropertyThreadSafe(AsynchronousServer.console, "Text", AsynchronousServer.console.Text + "E1 Client " + index + " (" + client.RemoteEndPoint.ToString() + "): Lost connection" + "\n");
                        ...
                        else
                        {
                            ...
                            Form1.SetControlPropertyThreadSafe(AsynchronousServer.console, "Text", AsynchronousServer.console.Text + "Client " + index + " (" + client.RemoteEndPoint.ToString()  + "): " + response);
